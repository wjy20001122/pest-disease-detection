...
def associate(detections, trackers, iou_threshold, velocities, previous_obs, vdc_weight):
    ...
    det_cx = (detections[:, 0] + detections[:, 2]) / 2.0
    det_cy = (detections[:, 1] + detections[:, 3]) / 2.0
    
    trk_cx = (previous_obs[:, 0] + previous_obs[:, 2]) / 2.0
    trk_cy = (previous_obs[:, 1] + previous_obs[:, 3]) / 2.0
    ...
    max_dist = 50.0
    hard_dist_threshold = 80.0
    dist_mask = center_dist <= hard_dist_threshold
    
    valid_mask_2d = valid_mask.T
    center_dist_penalty = (center_dist / max_dist) * valid_mask_2d * 0.5
    
    iou_matrix_filtered = iou_matrix * dist_mask

    if min(iou_matrix.shape) > 0:
        a = (iou_matrix_filtered > iou_threshold).astype(np.int32)
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            matched_indices = np.stack(np.where(a), axis=1)
        else:
            matched_indices = linear_assignment(-(iou_matrix_filtered + angle_diff_cost - center_dist_penalty))
    else:
        matched_indices = np.empty(shape=(0,2))

    return _filter_matches(matched_indices, iou_matrix_filtered, iou_threshold, len(detections), len(trackers))
